...
export default function Header() {
  ...
  const { user, profile, avatarUrl, refreshProfile } = useUser()
  ...
  }, [isAuthPage, user, profile, refreshProfile])
            ...
            <div className="flex items-center gap-2">
              <Sheet open={open} onOpenChange={setOpen}>
                ...
                <SheetContent side="left" className="flex flex-col p-6">
                  ...
                  {/* Display User Info on Mobile when available */}
                  ...
                  <nav className="flex flex-col gap-4 py-4">
                    {(isUserLoggedIn ? userNavigation : navigation).map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className={cn(
